...
class CommonsSim:
    ...
    def step(self, tax_rate: float):
        ...
        individual_harvests = [a*self.H for a in harvest_rates]
        ...
        obs = dict(
            field_health=self.H,
            avg_harvest=sum(harvest_rates)/self.n_agents,
            avg_reward=sum(rewards)/self.n_agents
        )
        info = dict(
            harvest_rates=harvest_rates, rewards=rewards, redistribution=redistribution,
        )
